...
// Una conversacion por contacto por linea (migracion 0108): se reutiliza el
// hilo mas reciente del contacto (responderle por donde escribio) o se crea
// con la linea por defecto del espacio.
async function getOrCreateConversation(
  supabase: SupabaseClient,
  workspaceId: string,
  contactId: string
): Promise<string | null> {
  const conversacion = await abrirConversacion(supabase, workspaceId, contactId);
