...
export function ConnectPlatformWhatsAppButton() {
  ...
  return (
    <>
      ...
      >
        {status === "connecting" ? "Conectando..." : "Conectar WhatsApp de administración"}
      </button>

      {message && (
        <p className={`mt-3 text-sm ${status === "error" ? "text-red-400" : "text-success"}`}>
          {message}
        </p>
      )}
    </>
